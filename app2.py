...
def save_embeddings_db(batch_id, file_name, window_starts, emb_array):
    ...
    cur.executemany(
        'INSERT INTO embeddings (batch_id, file_name, window_start, embedding) '
        'VALUES (?,?,?,?)', rows
    )
    con.commit()
    con.close()
...
def load_detections_db(batch_id=None):
    ...
    if batch_id and batch_id != 'All':
        df = pd.read_sql(
            'SELECT * FROM detections WHERE batch_id=?', con, params=(batch_id,)
        )
    else:
    ...
    cur.execute("""
        INSERT INTO detections
            (batch_id, file_name, start_time, end_time, common_name,
             scientific_name, confidence, lat, lon, rec_date,
...
# =============================================================================
# SONOGRAM HELPERS  (from original app)
# =============================================================================

def plot_annotated_sonogram(y, sr, detections, filename,
                             t_start_sec=0, t_end_sec=60):
    """Mel spectrogram with coloured detection overlays and legend."""
    ...
    window_dets  = [d for d in detections
                    if d['end_time'] >= t_start_sec
                    and d['start_time'] <= t_end_sec]
    ...
    if species_list:
        patches = [mpatches.Patch(color=cmap[sp], label=sp)
                   for sp in sorted(species_list)]
        ax.legend(handles=patches, loc='upper right', fontsize=7,
                  framealpha=0.6, ncol=max(1, len(species_list) // 6))

    ax.set_title(f'{filename}  [{t_start_sec:.0f}s – {t_end_sec:.0f}s]',
                 fontsize=10, pad=8)
...
def plot_detection_sonogram(y, sr, det, pad=1.0):
    """Compact spectrogram for a single detection (used in validation)."""
    ...
    S    = librosa.feature.melspectrogram(y=y_snip, sr=sr, n_mels=128)
    S_db = librosa.power_to_db(S, ref=np.max)
    librosa.display.specshow(S_db, x_axis='time', y_axis='mel',
                             sr=sr, ax=ax, cmap='magma')
    # Highlight the detection window
    ax.axvspan(pad, pad + (det['end_time'] - det['start_time']),
...
# =============================================================================
# AUDIO CLIP HELPERS  (from v2)
# =============================================================================

def clip_to_b64(file_path, start_sec, end_sec, sr=32000):
    """Return a base64 WAV data URI for an HTML <audio> tag."""
    y, _ = librosa.load(file_path, sr=sr, offset=start_sec,
...
def inline_player(file_path, start_sec, end_sec):
    try:
        ...
        st.markdown(
            f'<audio controls style="width:100%;" src="{uri}"></audio>',
...
def run_perch_classify(file_path, top_n=10):
    """
    Run Perch classification on a file.
    Scores are softmax-normalised within each window so they sit in [0,1]
    and behave consistently with BirdNET confidence values.
    Returns a list of detection dicts in the same shape as BirdNET output.
    """
...
def find_similar(query_start, window_starts, emb_matrix, top_n=10):
    ...
    distances = cdist(emb_matrix[query_idx:query_idx+1],
                      emb_matrix, metric='cosine')[0]
...
if PERCH_AVAILABLE:
    ...
    #         'Also generate Perch embeddings (Find Similar)',
    #         value=False,
    #         help='Adds ~30 s per hour of audio.')
    # else:
    #     use_perch = True
    #     st.sidebar.caption('Perch embeddings generated automatically.')
else:
...
# =============================================================================
# TAB 1 — ANALYSIS
# File upload → preview → BirdNET + optional Perch → results dashboard
#   • top-5 metric cards
#   • scrollable annotated sonogram explorer
#   • detection timeline + confidence box plots
#   • colour-gradient raw detections table
#   • Find Similar sounds (Perch)
# =============================================================================

with tab_analysis:

    # Optional: save this interactive session to the persistent database
    ...
    if uploaded_files:
        ...
        selected_prev = st.selectbox('Select a file to preview:', file_names)
            ...
            try:
                ...
                win_start   = (
                    st.slider('Scroll through recording:',
                              ...
                              value=0.0, step=float(window_size // 2),
                              format='%.0fs')
                    if max_start > 0 else 0.0)
                win_end     = min(win_start + window_size, total_dur)
                s0, s1      = int(win_start * sr_full), int(win_end * sr_full)
                ...
                ax_p.set_title(
                    ...
                    f'[{win_start:.0f}s–{win_end:.0f}s / {total_dur:.0f}s]',
                    fontsize=10)
        ...
        if st.button(run_label, key='run_analysis'):
            all_detections = []
            audio_cache    = {}   # fname -> (y, sr)
            emb_cache      = {}   # fname -> (window_starts, matrix, tmp_path)
            analyzer_obj   = load_analyzer()
            ...
            for i, uf in enumerate(valid_files):
                ...
                try:
                    ...
                    if model_choice in ('BirdNET', 'Both'):
                        dets_bn = run_birdnet(
                            tmp_path, lat, lon, conf_threshold,
                    ...
                    #             tmp_path = None   # keep alive for clip extraction
                    #     except Exception as e:
                    #         st.warning(
                    #             f'Perch embeddings failed for {uf.name}: {e}\n'
                    #             'BirdNET detections are still available.')
            ...
            if all_detections:
                # Initialise validation status for new detections
            ...
            if save_to_db and all_detections:
                ...
                if use_perch:
                    for fname, (ws, em, _) in emb_cache.items():
                        save_embeddings_db(session_batch_id, fname, ws, em)
                st.success(f"Saved to database under batch '{session_batch_id}'.")

    # --- RESULTS DASHBOARD ---
    if ('all_detections' in st.session_state
            and st.session_state['all_detections']):
        ...
        # Rebuild df with live validation statuses
        df = pd.DataFrame(all_detections)
        df['validation'] = df.apply(
            lambda r: get_vstatus(f"{r['file_name']}|{r['start_time']}"),
            axis=1)

        st.success(
            f'Analysis complete: {len(df)} detections '
            f'across {len(valid_file_names)} file(s).')

        # ── TOP-5 METRIC CARDS ──────────────────────────────────────────────
        st.subheader('Top Species by Confidence')
        top_sp = (df.groupby('common_name')['confidence']
                    .max().sort_values(ascending=False).head(5))
        card_cols = st.columns(min(5, len(top_sp)))
        for col, (species, conf) in zip(card_cols, top_sp.items()):
            count = len(df[df['common_name'] == species])
            col.metric(label=species, value=f'{conf:.0%}',
                       delta=f'{count} detection(s)')

        # ── ANNOTATED SONOGRAM EXPLORER ─────────────────────────────────────
        ...
        if sono_file in audio_cache:
            ...
            sono_win        = st.select_slider(
                'Window size (s):', options=[30, 60, 120, 300],
            ...
            fig_sono = plot_annotated_sonogram(
                y_sono, sr_sono, visible_dets, sono_file,
                t_start_sec=sono_start, t_end_sec=sono_end)
            st.pyplot(fig_sono)
            plt.close(fig_sono)
            in_win = [d for d in visible_dets
                      if d['end_time'] >= sono_start
                      and d['start_time'] <= sono_end]
            st.caption(
                f'Showing {len(in_win)} detection(s) in window. '
        ...
        tl_fig = px.scatter(
            ...
            hover_data={'file_name': True, 'confidence': ':.2f',
                        ...
                        'common_name': False},
            labels={'start_time': 'Time from Recording Start (s)',
                    'common_name': 'Species'},
        ...
        filtered = df[
            df['common_name'].isin(sp_filt) &
            df['file_name'].isin(fi_filt)]
        ...
        # # ── FIND SIMILAR SOUNDS ─────────────────────────────────────────────
        # if use_perch and emb_cache:
        #     st.divider()
        #     st.subheader('🔍 Find Similar Sounds')
        #     st.caption(
        #         'Select a detection to search for acoustically similar '
        #         'windows across the file — including windows the classifier missed.')

        #     det_labels = [
        #         f"{r['file_name']} | {r['start_time']:.1f}s — "
        #         f"{r['common_name']} ({r['confidence']:.2f})"
        #         for _, r in filtered.iterrows()
        #     ]
        #     if det_labels:
        #         sel_label = st.selectbox('Query detection:', det_labels)
        #         sel_idx   = det_labels.index(sel_label)
        #         qrow      = filtered.iloc[sel_idx]
        #         qfile     = qrow['file_name']
        #         qstart    = float(qrow['start_time'])
        #         qtop_n    = st.slider('Similar windows to return:', 5, 20, 10)

        #         if st.button('🔍 Search', key='sim_search'):
        #             emb_data = emb_cache.get(qfile)
        #             if emb_data is None:
        #                 st.warning(f'No embeddings for {qfile}.')
        #             else:
        #                 w_starts, emb_matrix, tmp_path = emb_data
        #                 similar = find_similar(
        #                     qstart, w_starts, emb_matrix, top_n=qtop_n)
        #                 st.session_state['similar_results'] = {
        #                     'results':   similar,
        #                     'tmp_path':  tmp_path,
        #                     'query_row': qrow.to_dict(),
        #                     'qfile':     qfile,
        #                 }

        #         sim_data = st.session_state.get('similar_results')
        #         if sim_data and sim_data.get('qfile') == qfile:
        #             similar  = sim_data['results']
        #             tmp_path = sim_data['tmp_path']
        #             qrow_d   = sim_data['query_row']

        #             st.write(
        #                 f"**{len(similar)} windows most similar to "
        #                 f"{qfile} @ {qstart:.1f}s ({qrow_d['common_name']}):**")

        #             for _, row in similar.iterrows():
        #                 win_key    = f"sim|{qfile}|{row['window_start']}"
        #                 cur_status = get_vstatus(win_key)
        #                 icon = ('✅' if cur_status == STATUS_ACCEPTED
        #                         else '❌' if cur_status == STATUS_REJECTED
        #                         else '⬜')

        #                 with st.expander(
        #                     f"{icon} {row['window_start']:.1f}s — "
        #                     f"{row['window_end']:.1f}s  |  "
        #                     f"Similarity: {row['similarity_pct']:.1f}%"):

        #                     inline_player(
        #                         tmp_path, row['window_start'], row['window_end'])

        #                     notes_val = st.text_input(
        #                         'Species / notes:',
        #                         key=f'notes_{win_key}',
        #                         placeholder='e.g. Curlew — faint call')

        #                     ca2, cb2, cc2 = st.columns(3)
        #                     with ca2:
        #                         if st.button('✅ Accept', key=f'acc_{win_key}'):
        ...
        #                                     common_name=notes_val or qrow_d['common_name'],
        #                                     scientific_name=qrow_d.get('scientific_name', ''),
        ...
        #                             st.rerun()
        #                     with cb2:
        #                         if st.button('❌ Reject', key=f'rej_{win_key}'):
...
# =============================================================================
# TAB 2 — VALIDATE
# One-at-a-time navigator with audio player + spectrogram per detection.
# Accept / Reject / Unsure / Skip.  Writes back to session state and optionally
# to the database.  Supports both session detections and pipeline DB batches.
# =============================================================================
...
with tab_validation:
    ...
    val_source = st.radio(
        'Validate from:',
        ['Current session', 'Pipeline database'],
        horizontal=True)

    # ── SOURCE A: current session ───────────────────────────────────────────
        ...
        if not all_dets:
            ...
            with vc2:
                val_st_filt = st.multiselect(
                    'Show status:',
                    options=[STATUS_UNREVIEWED, STATUS_ACCEPTED,
                             STATUS_REJECTED, STATUS_UNSURE],
            ...
            if queue.empty:
                ...
                with left_col:
                    ...
                    if b1.button('✅', key=f'acc_{det_key}', help='Accept'):
                        ...
                        if save_to_db:
                            update_verified_db(
                                det.get('id', -1), STATUS_ACCEPTED, notes_val)
                    ...
                    if b2.button('❌', key=f'rej_{det_key}', help='Reject'):
                        ...
                        if save_to_db:
                            update_verified_db(
                                det.get('id', -1), STATUS_REJECTED, notes_val)
                        st.session_state['val_idx'] = min(idx + 1, len(queue) - 1)
                        st.rerun()
            ...
            st.download_button(
                ...
                file_name='birdnet_detections_validated.csv',
                mime='text/csv')

    # ── SOURCE B: pipeline database ─────────────────────────────────────────
    else:
        ...
        if not batch_ids:
            ...
            db_st_filt = st.multiselect(
                ...
                key='db_st_filt')
            db_queue = db_df[
                db_df['verified'].isin(db_st_filt)
            ].reset_index(drop=True)
            ...
            if db_queue.empty:
                ...
                db_idx = max(
                    0, min(st.session_state['db_val_idx'], len(db_queue) - 1))
                ...
                st.markdown(
                    ...
                    f"<small style='color:grey;'>confidence "
                    f"{db_row['confidence']:.2f}</small>",
                    unsafe_allow_html=True)
                st.caption(
                    f"**Batch:** {db_row['batch_id']}  |  "
                    f"**File:** {db_row['file_name']}  |  "
                    f"**Time:** {db_row['start_time']:.1f}s — "
                    f"{db_row['end_time']:.1f}s  |  "
                    f"**Source:** {db_row.get('source', 'birdnet')}")

                st.info(
                    '💡 Audio playback for pipeline results requires the original '
                    'file to be present. Re-upload via the Analysis tab if needed.')

                db_notes = st.text_input(
                    'Notes:', key=f"dbnotes_{db_row['id']}",
...
# Headless processing of large ARU batches, results to DB.
# =============================================================================
...
with tab_batch:
    ...
    st.caption(
        'Process large batches of ARU recordings headlessly. '
        ...
        if st.button('▶️ Run Batch', key='run_batch'):
            ...
            for i, uf in enumerate(batch_files):
                ...
                with tempfile.NamedTemporaryFile(
                        suffix=Path(uf.name).suffix, delete=False) as tmp:
                    tmp.write(uf.getbuffer())
                    tmp_path = tmp.name

                try:
                    dets = run_birdnet(
                        tmp_path, b_lat, b_lon, b_conf,
            ...
            st.success(
                f'✅ Batch complete — {det_count} detections from '
                f'{n_ok} / {len(batch_files)} files.')
            if error_log:
...
# Browse, filter, chart, and export the full database.
# =============================================================================
...
with tab_results:
    ...
    if all_db.empty:
        ...
        view_df = (
            all_db[all_db['batch_id'] == sel_batch_r].copy()
            if sel_batch_r != 'All'
            else all_db.copy())

        st.caption(
            f"{len(view_df)} detections  |  "
        ...
        with rc1:
            r_sp = st.multiselect(
                'Species:', sorted(view_df['common_name'].unique()), default=[])
        with rc2:
            r_st = st.multiselect(
                'Status:',
        ...
        with st.expander('📈 Summary charts'):
            ...
            with pc1:
                ...
                st.plotly_chart(
                    px.bar(top20, x='count', y='common_name',
                           orientation='h', title='Top 20 species'),
                    use_container_width=True)
            with pc2:
                st.plotly_chart(
                    px.pie(
                        view_df, names='verified',
        ...
        st.download_button(
            ...
            file_name=f'pipeline_{sel_batch_r}.csv',
            mime='text/csv')
